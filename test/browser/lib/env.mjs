 ...
 * plugin measures its own container, and the probe is removed again. The class ties the probe
 * to the plugin's stylesheet, which the fixture always loads: the width and display it is
 * measured with are the ones the slider's own .irs spans get. matrix.spec.mjs checks the
 * answer against the slider itself on every run.
 *
