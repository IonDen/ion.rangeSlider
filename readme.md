...
| `prettify` | `data-prettify` | `null` | function, string | Custom formatting function. Receives a number, returns a string (with `prettify_all_values`, non-numeric `values` entries are passed too). A string instead names a global function (`window[name]`); an unresolved name falls back to the default formatting |
