...
* **jQuery 3.0.x and 3.1.x:** these builds include the rotation of the `square` skin's handle in its width, so they read it as about 23 px instead of 16, and a drag reports a value up to about 1% of the slider's range too high (a drag to the middle of a 0 to 100 slider reports 51). jQuery 3.2.1 or newer measures it correctly. The other five skins are not affected.
