...
Node 22 or newer is needed to build and test (consumers need nothing: `js/` and `css/` are committed). `npm install`, then `npm run build` regenerates the three built files (`css/ion.rangeSlider.css` and the two minified files) from `less/` and `js/ion.rangeSlider.js`; `npm test` runs the unit tests. A pull request must not include rebuilt files: CI builds from source before running the tests and rejects a hand-edited or half-rebuilt built file. The built files on master are refreshed automatically after every merge (a "Rebuild dist" commit from the GitHub Actions bot) and are also regenerated by the release procedure; they are not part of any individual pull request. Before running the browser suite locally (`npm run test:browser`), run `npm run build` so it exercises your source rather than master's last build, and leave the result out of your commit.
